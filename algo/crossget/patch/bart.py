...
# from transformers.models.bart.modeling_bart import BartModel, BartEncoder, BartEncoderLayer, BartAttention,  shift_tokens_right
# from transformers.modeling_outputs import BaseModelOutput, Seq2SeqModelOutput, dataclass, ModelOutput 
# from typing import Optional, Tuple, Union, List
# from ..merge import merge_source, cross_get_text, merge_wavg, merge_attention_mask
...
# class CrossGetBartEncoderLayer(BartEncoderLayer):
...
#         if ratio < 1.0:
#             merge, isolated_score = cross_get_text(
#                 ratio=ratio,
#                 metric=metric,
#                 margin=self.margin,
...
# class CrossGetBartAttention(BartAttention):
...
#             H,T,C = key_states.shape
#             # key_states = key_states.view(T, -1)
#             key_states = key_states.sum(0)
#         else:
#             B,H,T,C = key_states.shape
#             # key_states = key_states.view(B, T, -1)
#             key_states = key_states.sum(1)

#         return attn_output, attn_weights_reshaped, key_states, past_key_value

# class CrossGetBartEncoder(BartEncoder):
#     """
#     Modifications:
...
# def make_cross_get_class(transformer_class:BartModel):
#     class CrossGetBartModel(transformer_class):
#         def init_r(self):
...
#     """
#     CrossGetBartModel = make_cross_get_class(model.__class__)
#     print('using', 'cross_get')

#     model.__class__ = CrossGetBartModel
...
#     margins = [.75 - 0.25*(i/num_layers) for i in range(num_layers)]

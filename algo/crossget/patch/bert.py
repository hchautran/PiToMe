...
from ..merge import merge_source, crossget, merge_mean, merge_wavg, merge_attention_mask
from transformers.modeling_utils import ModuleUtilsMixin 
from typing import Optional, Union 
import math


class CrossGetBertLayer(BertLayer):
...
class CrossGetBertAttention(BertAttention):
...
class CrossGetBertSelfAttention(BertSelfAttention):
...
def make_cross_get_class(transformer_class):
    class CrossGetBertEncoder(transformer_class, ModuleUtilsMixin):
        """
        Modifications:
    ...
    return CrossGetBertEncoder



def apply_patch(
   model: BertEncoder, trace_source: bool = False, prop_attn: bool = True, use_attn=False):

    CrossGetBertEncoder = make_cross_get_class(model.__class__)
    print('using', 'cross_get')

    model.__class__ = CrossGetBertEncoder
    model.ratio = 1.0 
    ...
    for module in model.modules():
        if isinstance(module, BertLayer):
            module.__class__ = CrossGetBertLayer
            module._info = model._info
        if isinstance(module, BertAttention):
            module.__class__ = CrossGetBertAttention 
        if isinstance(module, BertSelfAttention):
            module.__class__ = CrossGetBertSelfAttention 
